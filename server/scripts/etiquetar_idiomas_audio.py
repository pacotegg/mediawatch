"""
PASADA 3 de 3 (accion): pone el idioma a las pistas de audio sin etiquetar,
...
INVENTARIO = r"C:\tvwatch\data\inventario-idiomas-audio.jsonl"
REGISTRO = r"C:\tvwatch\data\etiquetado-idiomas-audio.jsonl"
...
def confiada(p):
    return (p.get("prob") is not None and p.get("votos") == p.get("tramos")
            and p["prob"] >= PROB_MINIMA and p.get("iso3"))
...
def leer(path):
    ...
    audio = {t["properties"].get("uid"): (t["properties"].get("language"),
             ...
             for t in j.get("tracks", []) if t.get("type") == "audio"}
